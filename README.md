...
### Цвета граней и настройка металлов

Базовые цвета настраиваются в начале файла `main.js` в словаре `CONFIG.faceColors`:

```js
// main.js → CONFIG.faceColors
faceColors: {
  right: '#9760D3',   // фиолетовый (+X)
  left: '#E6E8EA',    // серебряный (-X)
  top: '#F67629',     // оранжевый (+Y)
  bottom: '#ffffff',  // чисто белый (-Y)
  front: '#B9FF00',   // зелёный (+Z)
  back: '#3498db',    // синий (-Z)
},
```

#### Зеркальные и металлические текстуры
Проект поддерживает PBR (физически корректный рендеринг) со студийным освещением (`RoomEnvironment`). Чтобы превратить любую грань в металлик или добавить текстуру брашированного (тёртого) металла, добавьте её HEX-код в `MATERIAL_PROPS` (ниже в `main.js`):

```js
// main.js → MATERIAL_PROPS
const MATERIAL_PROPS = {
  '#E6E8EA': { roughness: 0.18, metalness: 0.85, shine: true },
};
```

**Параметры:**
- `metalness` *(0.0–1.0)*: сила отражения студийного света (0.85 — 0.95 даёт настоящий хром/зеркало).
- `roughness` *(0.0–1.0)*: шероховатость (отражения либо четкие глянцевые `0.1`, либо мутные и матовые `0.6`).
- `shine: true`: включает генерацию фейковых процедурных царапин (brushed grain) и карты рельефа (Bump Map) для создания эффекта виниловой наклейки на зеркальный Mirror Cube. Без флага `shine` материал будет просто идеально гладким зеркалом.
